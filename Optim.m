...
bw = [-0.25 -0.15 0.25];            % Offset to the central frequency
...
datos(1,4) = freq;
h = 10;         % Height of the waveguide (mm). Evaluate different values.
datos(1,1) = h;
...
options=optimset('Algorithm','active-set','Maxiter',5e3,'MaxFunEvals',5e3,'PlotFcns', {@optimplotx, @optimplotfval,@optimplotfunccount,@optimplotconstrviolation});
...
path = 'AntennasSaved/';
save ([path filename '.mat'], 'datos', 'puntos', 'cortos', 'nslotsSol', 'sondas', 'slotsSol', 'dirSoldB', 'dirXPdB', 'datos', 'deltaRsol', 'longcsol', 'varPossol', 'fval', 'exitflag', 'output', 'resPhi', 'resTheta');
...
%% Repeat for fmin and fmax
load(strcat('AntennasSaved/', filename, '.mat'));
